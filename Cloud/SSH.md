...
Provide a useful file name, and an optional passphrase

```bash
ssh-add .ssh/id_ed25519
```

Add the key to your SSH Agent, this may require starting with

```bash
eval `ssh-agent`
```

Cat the **public** key file to get its contents

```bash
cat .ssh/id_ed25519.pub
```

Copy the output to the GitHub Deploy Key/SSH keys section in the settings

Run this command to validate the key works
...
```bash
Hi Dodecahedrane! You've successfully authenticated, but GitHub does not provide shell access.
```